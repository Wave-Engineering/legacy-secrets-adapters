...
<!--
  Copy this directory to bootstrap-pattern-demos/<your-pattern-name>/ (kebab-case) and fill in
...
- <e.g. "the materializer needs to authenticate to its key source without a stored secret">
...
This demo anchors the bootstrap secret (here: `<name the obvious dev value — e.g. a simulated TPM
handle>`). What *consumes* the anchored credential — a delivery pattern that materializes or streams
the secret to an unchanged app — is an orthogonal concern addressed by the delivery-pattern family
(see [`docs/decision-guide.md`](../../docs/decision-guide.md)). Conflating the two would obscure
